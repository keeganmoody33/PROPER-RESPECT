...
describe("private usage card projection", () => {
  it("includes native-only and mixed reports without silently dropping evidence", () => {
    ...
    for (const inputs of [[native], [native, text("priced-synthetic")]]) {
      ...
      expect(preview.tools[0].nativeClaude?.rows[0].quantity).toBe("5");
      expect(preview.tools[0].rows).toHaveLength(inputs.length - 1);
    }
  });
