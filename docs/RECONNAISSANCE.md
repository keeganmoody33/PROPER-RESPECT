...
- **Acceptance criteria:** A template-generated repo with independent history can deploy a branded site with local pages; mandated disclosure semantics are enforced centrally — public data is served only through the issue 6 contract with revocable access tokens for future reads/builds, and policy-sensitive fields stay out of static build artifacts unless a runtime invalidation path exists.
...
- **Security/privacy/SEO:** No privileged secrets in repository/browser; canonical host configuration; dependency/update ownership documented; owner-controlled static deployments are outside central enforcement, so takedown-sensitive data stays out of static output unless pages re-fetch it at runtime with short-lived revocable tokens.
