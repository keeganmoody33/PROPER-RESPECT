...
Against head `652f49734a992e44f9e8ae860c67261e2addeb46`, the full CI suite found an older sharing-label fixture that expected a claimed handle without supplying the new identity capability. Reproduced locally: 1 failed and 15 passed in `src/client/onboarding-auth.test.ts` (`/tmp/u2-ci-fixture-red.log`). The claimed fixture now explicitly supplies `hasClaimedPublicIdentity: true`. A separate absent-capability test proves preview remains disabled, the unavailable-state explanation appears, and neither claimed-handle publication wording nor a public-page link is fabricated. No UI or backend implementation changed.

Full `npm test` passes: **781 Vitest tests passed, 2 skipped; 7 Node script tests passed** (`/tmp/u2-ci-fixture-full-tests.log`). Full `npm run typecheck`, `npm run lint` and `git diff --check` pass. The only preexisting working-tree change was Next's two generated `.next/dev/types` imports; after inspection, they were restored to the exact tracked HEAD bytes as authorized. No account or provider reads, publication, backend synchronization or deployment occurred.
