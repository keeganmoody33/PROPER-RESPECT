...
Logs are retained privately at /Users/keeganmoody/Documents/PROPER-RESPECT-private/2026-09-21-agent-discovery/. The HTTP regression is committed in tests/e2e/discovery.spec.ts and is already included by the existing CI workflow. Native Chrome emitted React's development warning about non-executable script tags during client navigation; JSON-LD remains parseable on initial HTML and no runtime test failed.
...
| Agent Plugins manifest | Not applicable to current interface | No product plugin or bundled remote MCP server exists. A discovery document is not a plugin manifest. |
...
| A2A / agent-card | Not applicable to current interface | No A2A agent or contact endpoint exists. |
| MCP well-known discovery | Not applicable to current interface | Browser WebMCP is not a remote MCP transport. No fake server URL published. |
...
| API catalog (RFC 9727) | Not applicable to current interface | No supported external HTTP API or OpenAPI document exists to catalog. |
...
| NLWeb Schema Feeds | Not applicable to current interface | robots.txt added, but no JSONL/RSS Schema Feeds exist. Agentmap points to the real ARD catalog; no fake schemamap feed. |
...
| Bot-UA markdown serving | Declined optional behavior | No User-Agent cloaking. All clients can request the same explicit Markdown resources. |
...
| MCP Apps support | Not applicable to current interface | No remote MCP server or ui:// resource exists. |
| Agent auth discovery metadata | Not applicable to current interface | No supported agent credential exchange exists. Clerk human sessions are not a public OAuth agent authorization server. |
| Web Bot Auth directory | Not applicable to current interface | The product is not operating an agent signing-key service. No fabricated keys or unverified signing claims. |
| OAuth Protected Resource metadata (RFC 9728) | Not applicable to current interface | No OAuth-protected external agent API exists. Do not advertise internal Clerk/Convex credentials. |
...
| auth.md structure | Not applicable to current auth | No register/claim/exchange/ID-JAG flow is offered. auth.md explicitly states the unsupported methods instead of adding misleading spec keywords. |
| agent_auth endpoints reachable | Not applicable to current auth | No agent_auth endpoints are advertised; no placeholder endpoints created to evade 404 probes. |
| MCP server-card.json | Not applicable to current interface | No remote MCP server exists to describe. |
...
| Agent auth WWW-Authenticate hint | Not applicable to current auth | No supported bearer-token API exists. Do not manufacture 401 discovery headers on valid human profile routes. |
| NLWeb /ask endpoint | Not applicable to current interface | No NLWeb query service exists. Adding one would require a separate data/access and cost design. |
| NLWeb streaming support | Not applicable to current interface | No NLWeb endpoint exists to stream. |
