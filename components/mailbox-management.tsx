...
export function MailboxManagement() {
  ...
  return <div className="connector-card" aria-labelledby="gmail-management-title">
    ...
    {accounts === undefined && isAuthenticated && <p>Loading your mailbox connections…</p>}
    {accounts?.filter(account => account.provider === "GOOGLE").map(account => <article key={account.accountId} style={{ borderTop: "1px solid currentColor", paddingBlock: "1rem", overflowWrap: "anywhere" }}>
      ...
      <div className="action-row">
        {modes.map(({ mode, action, continueAction, restartAction }) => {
          const progress = account.contexts?.find(context => context.mode === mode);
          return progress?.lastFailure === "CURSOR_EXPIRED" ? <button key={mode} className="secondary-action" disabled={busy || account.status !== "CONNECTED"}
            ...
            <button key={mode} className="secondary-action" disabled={busy || account.status !== "CONNECTED"}
      ...
      <details><summary>Automatic discovery and account controls</summary>
        ...
        <button className="secondary-action" disabled={busy || (account.status !== "CONNECTED" && !account.maintenanceEnabled)}
