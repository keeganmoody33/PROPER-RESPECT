...
export default defineSchema({
  ...
  manualProductIntakes: defineTable({
    userId: v.id("users"),
  ...
  props: defineTable({
    ...
    headline: v.string(),
    note: v.string(),
    ownerEntered: v.optional(v.boolean()),
    // Owner decisions, independent of measured activity and publication.
    goTo: v.optional(v.boolean()),
    confirmedAt: v.optional(v.string()),
    relationshipVersion: v.optional(v.number()),
  ...
  evidenceSources: defineTable({
    userId: v.id("users"),
    type: evidenceSourceTypeValidator,
    // Stable provider account identity, never a mutable email address or label.
    // Optional only for existing single-source import callers.
  ...
  rawEvidence: defineTable({
    ...
    suggestedActivity: v.optional(activityModuleValidator),
    retainedArtifact: v.optional(v.object({
      kind: v.union(v.literal("WISPR_INSIGHTS"), v.literal("WISPR_OWNER_REVIEW"), v.literal("GITHUB_ACTIVITY")),
      sourceFile: v.string(), sha256: v.string(), byteLength: v.number(),
      sourceCapturedDate: v.string(), sourceCaptureBasis: v.literal("RETAINED_SOURCE_DATE"),
      preparedAt: v.string(), adapterVersion: v.string(),
    })),
  ...
  claimReviews: defineTable({
    ...
    verdict: claimVerdictValidator,
    correction: v.optional(v.string()),
    reviewedAt: v.string(),
  })
    .index("by_prop", ["propId"])
    .index("by_prop_and_rawEvidenceId_and_observationIndex", ["propId", "rawEvidenceId", "observationIndex"]),
  ...
  proofs: defineTable({
    ...
    .index("by_prop", ["propId"])
    .index("by_propId_and_rawEvidenceId", ["propId", "rawEvidenceId"]),
