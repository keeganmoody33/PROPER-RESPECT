  ...
  handler: async (ctx, args) => {
    ...
    if (!prop) {
      const propId = await ctx.db.insert("props", {
        ...
        // Account creation belongs to activity.memberSince. It does not
        // establish when the owner began using this product.
    ...
    if (!draft) {
      await ctx.db.insert("draftImports", {
        ...
        rawEvidenceIds: snapshotEvidenceIds,
        resultPropId: prop._id,
      });
    } else if (draft.status === "PENDING") {
      await ctx.db.patch(draft._id, { rawEvidenceIds: [...draft.rawEvidenceIds, ...snapshotEvidenceIds] });
    }
...
export const applyRefresh = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!prop.activityEvidenceId && !prop.relationshipVersion) await ctx.db.patch(prop._id, { activity: args.activity });
    ...
    if (user && product) {
      ...
      if (published) {
        ...
        await ctx.db.patch(published._id, {
          ...
          profile: {
            ...published.profile,
            cards: published.profile.cards.map((card, index) =>
              indices.has(index)
      ...
      if (published) {
        ...
        await ctx.db.patch(published._id, {
          profile: {
            ...published.profile,
            cards: published.profile.cards.map((card, index) =>
              indices.has(index) && card.activity
                ? { ...card, activity: { ...card.activity, freshness: "STALE" as const } }
                : card,
            ),
          },
        });
