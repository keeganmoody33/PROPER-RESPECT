...
const failed = makeFunctionReference<"mutation">("connectors:markRefreshFailed");
const refreshed = makeFunctionReference<"mutation">("connectors:applyRefresh");
const activity = (total: number) => ({ kind: "contributionCalendar" as const, total, days: [],
  attributionScope: "PERSONAL" as const, capturedAt: "2026-09-18T10:00:00.000Z",
...
async function fixture(publicActivity = true) {
  ...
  const ids = await t.run(async ctx => {
    ...
    const connectorId = await ctx.db.insert("connectorAccounts", { userId, provider: "GITHUB", status: "CONNECTED", accountLabel: "Synthetic", attributionScope: "PERSONAL", connectedAt: "2026-09-18" });
    const subscriptionId = await ctx.db.insert("metricSubscriptions", { userId, propId, connectorId, metricKey: "github.contributions", attributionScope: "PERSONAL", refreshCadence: "DAILY", approvedAt: "2026-09-18" });
...
for (const publicActivity of [true, false]) test(`failed refresh preserves the approved public snapshot (activity=${publicActivity})`, async () => {
  ...
  await t.mutation(failed, { subscriptionId, message: "Synthetic temporary failure" });
...
test("successful approved refresh does not replace a privately selected retained source", async () => {
  ...
  const privateBefore = await t.run(ctx => ctx.db.get(propId));
  await t.mutation(refreshed, { subscriptionId, activity: activity(5), value: 5 });
...
test("an in-flight failure cannot reactivate a revoked connector or change its public snapshot", async () => {
  ...
  await t.mutation(failed, { subscriptionId, message: "Late failure" });
...
for (const succeeds of [true, false]) test(`refresh targets only its approved relationship when a product has two public cards (success=${succeeds})`, async () => {
  ...
  if (succeeds) await t.mutation(refreshed, { subscriptionId, activity: activity(5), value: 5 });
  else await t.mutation(failed, { subscriptionId, message: "Synthetic failure" });
